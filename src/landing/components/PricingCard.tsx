...
export default function PricingCard({ tier, price, unit, description, features, ctaText, featured = false }: PricingCardProps) {
  if (featured) {
    return (
      <div className="relative p-9 rounded-2xl flex flex-col h-full mt-4 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl bg-gradient-to-br from-brand-primary via-brand-primary to-brand-accent text-white shadow-xl shadow-brand-primary/25">
  ...
  return (
    <div className="relative p-[1px] rounded-2xl mt-4 bg-gradient-to-br from-emerald-200 via-gray-100 to-teal-200 transition-all duration-300 hover:-translate-y-1.5 group">
      <div className="relative p-9 rounded-[15px] flex flex-col h-full bg-white">
        <p className="text-xs font-semibold uppercase tracking-widest mb-4 text-text-muted">{tier}</p>
        <p className="text-4xl font-extrabold tracking-tight mb-2 text-text-heading">
          {price}
          {unit && <span className="text-base font-medium tracking-normal ml-1 text-text-muted">{unit}</span>}
        </p>
        <p className="text-sm leading-relaxed mb-8 pb-8 border-b border-border text-text-body">
          {description}
        </p>
        ...
        <ul className="flex flex-col gap-3.5 mb-8 flex-1">
          {features.map((feature, i) => (
            <li key={i} className="flex items-start gap-2.5 text-sm text-text-body">
              <span className="shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 bg-emerald-50 text-brand-primary">
                <Check size={12} />
              </span>
              {feature}
            </li>
        ...
        <a
          href="#cta"
          className="w-full inline-flex items-center justify-center py-3.5 rounded-full font-semibold text-sm transition-all duration-300 bg-brand-primary text-white hover:bg-emerald-600 shadow-md shadow-brand-primary/20"
        >
          {ctaText}
        </a>
      </div>
